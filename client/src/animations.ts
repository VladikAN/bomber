...
const loopAnimation = (scene: Phaser.Scene, name: string, frames: number[]): Phaser.Types.Animations.Animation => {
    return {
        key: name,
...
const singleAnimation = (scene: Phaser.Scene, name: string, frames: number[]): Phaser.Types.Animations.Animation => {
    return {
        key: name,
        frames: scene.anims.generateFrameNumbers('sprite', { frames: frames }),
        frameRate: Consts.animationFrameRate,
