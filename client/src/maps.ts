...
export const loadMap = (scene: Phaser.Scene): Phaser.Tilemaps.TilemapLayer[] => {
    ...
    let levelLayer = map.createLayer(0, tileset, Consts.spriteFrame, Consts.spriteFrame);
    levelLayer.setCollision([Tiles.wall, Tiles.brick]);
    
    // Create rect around the level
    let edgeLayer = map.createBlankLayer('edge', tileset, 0, 0, map.width + 2, map.height + 2);
